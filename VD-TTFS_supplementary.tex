...
The integrator partitions each layer's firing window---$W$ timesteps wide,
truncated to $[0,T'_\ell)$---into chunks of
...
\begin{itemize}
  \item A \textbf{finer} $\Delta$ lets a neuron stop at a finer temporal
    ...
    loop overhead), repeated $\lceil W/\Delta\rceil$ times, so a very small
...
the arrival-ordered early-exit factor $\gamma$---and on the per-layer window $W$:
...
For each task we sweep $\Delta$ over powers of two up to the per-layer window $W$,
...
\paragraph{Per-layer window versus total simulation length.}
The window $W$ chunked above is the window within which a \emph{single layer's}
neurons fire, which is distinct from a network's total simulation length $T$ (the
value reported in the table captions below). The T2FSNN-derived networks
(Tasks~1--2) are depth-staggered: every layer fires within a local window of $W=80$
timesteps, and successive layers are offset by a fixed inter-layer delay of $40$,
each layer's events being re-based to $[0,W)$ before chunking. For the shallow,
effectively un-staggered LeNet (Task~1) the two coincide, $T=80$. For VGG-16
(Task~2) the offsets accumulate across its sixteen weight layers---%
$15\times 40 + 80 = 680$---so $T=680$, even though each layer is still integrated
within its own $80$-step window. The DVSGesture network (Task~3) is not staggered,
so $W=T=160$. Consequently $\Delta$ partitions the per-layer window $W$, and the
sweeps below range only up to $W$ ($\le 80$ for Tasks~1--2, $\le 160$ for Task~3)---%
never up to VGG-16's $680$.
...
\begin{table}[t]
\centering
\caption{Task~2, VGG-16/CIFAR-10 ($T=680$), full 10{,}000-image test set.}
\label{tab:chunk-t2}
\begin{tabular}{cccc}
\toprule
$\Delta$ & Accuracy & Inference time (s) & \\
