// MAC-count instrumentation for the VD-TTFS time-sorted early-exit integrator.
// Task 1: LeNet on MNIST. Reports sparse SNN MACs vs dense ANN MACs.
// Build: nvcc stat_mnist_macs.cu -o stat_mnist_macs -O3 -Wno-deprecated-gpu-targets
//        (add -DNOEARLYEXIT to disable early exit). Run: ./stat_mnist_macs
#include <iostream>
#include <vector>
#include <string>
#include <fstream>
...
// ---- counting-sort each input column's active spikes into arrival chunks ----
__global__ void k_bucketize(const float* __restrict__ in, int N_cols, int C_in,
    ...
    for (int i = 0; i < C_in; ++i) {
        ...
        if (t_in < INF_TIME) {
            ...
            if (tb < TIME_WINDOW) cnt[tb / CHUNK]++;
        }
    }
...
// ---- chunked conv/fc with input-generation early-exit ----
__device__ unsigned long long g_mac_processed = 0;
    ...
    for (int tau = 0; tau < NUM_CHUNKS; ++tau) {
        ...
        if (is_fc) {
            ...
            for (int idx = s; idx < e; ++idx) {
                ...
                int tb = (int)floorf(t_in - TIME_FIRE_START); if (tb < 0) tb = 0;
                delta[tb - base_t] += kernel[(size_t)cin * C_out + c_out] * LUT_decay[tb]; ++my_macs;
            }
        } else {
            // valid convolution, stride 1, no padding: h_in = h_out + kh
            for (int kh = 0; kh < K_h; ++kh) {
                ...
                for (int kw = 0; kw < K_w; ++kw) {
                    ...
                    for (int idx = s; idx < e; ++idx) {
                        int cin = bc[idx]; float t_in = bt[idx];
                        int tb = (int)floorf(t_in - TIME_FIRE_START); if (tb < 0) tb = 0;
                        delta[tb - base_t] += kernel[(size_t)(wbase + cin) * C_out + c_out] * LUT_decay[tb]; ++my_macs;
                    }
                }
            }
        }
...
#ifndef NOEARLYEXIT
        if (result < INF_TIME) break;   // fired -> skip remaining chunks
#endif
    }
...
static unsigned long long g_dense_macs = 0;
...
void launch_layer(float* d_in, float* d_out, int H_in, int W_in, int C_in, int H_out, int W_out, int C_out,
    ...
    int N_cols = is_fc ? B : (B * H_in * W_in);
...
int main() {
    ...
    // bucket buffers, sized for the largest layer input
    size_t MAX_ELEMS = (size_t)B * 12 * 12 * 12;   // conv2 input is the largest N_cols*C_in
    size_t MAX_COLS  = (size_t)B * 28 * 28;        // conv1 input has the most columns
    ...
    std::cout << ">>> Running time-sorted early-exit inference (CHUNK=" << CHUNK << ")..." << std::endl;
    ...
    // Conv1: 28x28x1 -> 24x24x12   (integ = input encoding constants)
    ...
    // Conv2: 12x12x12 -> 8x8x64    (integ = layer0 fire constants)
    ...
    // FC: 1024 -> 10               (integ = layer1 fire constants)
    ...
    std::cout << "\n=======================================" << std::endl;
    std::cout << "TIME-SORTED EARLY-EXIT (LeNet-MNIST) COMPLETED" << std::endl;
    ...
    unsigned long long mp=0; CHECK_CUDA(cudaMemcpyFromSymbol(&mp,g_mac_processed,sizeof(mp)));
    double dense=(double)g_dense_macs;
    std::cout << "\n--- OPERATION COUNT (conv+fc) ---" << std::endl;
#ifdef NOEARLYEXIT
    std::cout << "MODE: no early-exit -> Fr*dense" << std::endl;
#else
    std::cout << "MODE: early-exit ON -> Fr*gamma*dense" << std::endl;
#endif
    std::cout << "Dense ANN MACs: " << dense << std::endl;
    std::cout << "SNN sparse MACs: " << (double)mp << std::endl;
    std::cout << "SNN/ANN ratio: " << (mp/dense) << std::endl;
    std::cout << "Ops saved: " << 100.0*(1.0-mp/dense) << " %" << std::endl;
    std::cout << "=======================================" << std::endl;
    return 0;
}
